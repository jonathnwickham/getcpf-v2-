...
const STATUS_STYLES = {
  pass: { bg: "bg-primary/10", text: "text-primary", icon: "✓", label: "Pass" },
  warning: { bg: "bg-amber-100 dark:bg-amber-950/30", text: "text-amber-700 dark:text-amber-300", icon: "⚠", label: "Needs attention" },
      ...
      <div className="p-6">
        ...
        {isAnalyzing && (
          <div className="mt-6 text-center py-8">
            <div className="inline-flex items-center gap-3 bg-primary/10 text-primary px-5 py-3 rounded-xl">
              <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin" />
              <span className="text-sm font-semibold">Analyzing {fileName}…</span>
            </div>
            <p className="text-xs text-muted-foreground mt-3">Our AI is checking your document against CPF requirements</p>
          </div>
        )}
        ...
        {result && overallStyle && (
          <div className="mt-6 space-y-4 animate-slide-in">
            ...
            <div className={`flex items-center gap-3 ${overallStyle.bg} rounded-xl p-4`}>
              {preview && (
                <img src={preview} alt="Document" className="w-16 h-16 object-cover rounded-lg border border-border" />
              )}
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs bg-card border border-border px-2 py-0.5 rounded font-medium">
            ...
            {result.checks.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Checklist</p>
