...
const DiagnosisScreen = ({ cause, location, data, motherNameOverride, onMotherNameChange, onGoToActionPlan, onGoToDocuments }: DiagnosisProps) => {
      ...
      <div className="space-y-4">
        ...
        {data?.motherName && (
          <div className="bg-gray-50 rounded-xl p-4">
            <p className="text-xs text-gray-500 font-semibold mb-1">Name you previously entered:</p>
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-white border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none focus:border-green-800 focus:ring-2 focus:ring-green-800/10"
          />
        </div>
      </div>
    ),
    B: (
      <div className="space-y-4">
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-2">
          ...
          <p className="text-xs font-semibold text-green-800">💰 Cost: approximately R$30–50 per document</p>
        </div>
    ...
    C: (
      <div className="space-y-4">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
          ...
          <p className="text-sm text-gray-500">Here's exactly what counts as valid proof of address:</p>
        </div>
        <div className="space-y-2">
          {[
            ...
            <div key={doc.title} className="bg-gray-50 rounded-xl p-3 flex items-start gap-3">
        ...
        <div className="bg-green-800/5 border border-green-800/15 rounded-xl p-4">
          ...
          <p className="text-xs text-gray-500 mb-3">
            Your Ready Pack can generate this letter for you.
          </p>
          {onGoToDocuments && (
            <button onClick={onGoToDocuments} className="text-xs font-semibold text-green-800 hover:underline">
              Go to Documents tab →
            </button>
          )}
        </div>
    ...
    D: (
      <div className="space-y-4">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4 space-y-3">
          ...
          <div className="bg-green-800/5 rounded-xl p-3">
    ...
    F: (
      <div className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <span className="w-6 h-6 bg-green-800/10 text-green-800 rounded-full flex items-center justify-center shrink-0 text-xs font-bold">{i + 1}</span>
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-2">
          ...
          {[
            { status: "Regular", desc: "Your CPF is active. you're done! 🎉", color: "text-green-700" },
            { status: "Pendente de regularização", desc: "Action needed. contact your nearest Receita Federal office", color: "text-amber-700" },
            { status: "Suspensa", desc: "Suspended. visit Receita Federal in person with your documents", color: "text-amber-700" },
            { status: "Cancelada", desc: "Cancelled. visit Receita Federal. This can usually be reactivated", color: "text-red-500" },
          ].map((s) => (
            ...
            {[
              ...
              <button
                ...
                className={`w-full flex items-start gap-3 p-3 rounded-xl text-left transition-all ${
                  checkedFields.has(i) ? "bg-green-800/5 border border-green-800/10" : "bg-gray-50"
                }`}
              >
                <div className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold border-2 transition-all ${
                  checkedFields.has(i) ? "border-green-800 bg-green-800 text-white" : "border-gray-100"
                }`}>
                  {checkedFields.has(i) ? "✓" : ""}
                </div>
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <div className="bg-green-800/5 border border-green-800/15 rounded-2xl p-6">
        <span className="text-xs font-bold text-green-800 uppercase tracking-wider">Diagnosis</span>
        <h2 className="text-xl font-extrabold mt-1">{CAUSE_TITLES[cause]}</h2>
      </div>
      {content[cause]}
      <button
        onClick={onGoToActionPlan}
        className="w-full bg-green-800 text-white px-6 py-3.5 rounded-xl font-bold text-sm hover:opacity-90 transition-all"
      >
        See my action plan →
      </button>
    </div>
...
const RejectionFlow = ({ onClose, onGoToDocuments, data }: { onClose: () => void; onGoToDocuments?: () => void; data?: OnboardingData | null }) => {
  ...
  if (screen === "action" && rootCause) {
    ...
    // Extract "Bring:" items from steps for the summary card
    const bringStep = plan.steps.find(s => s.toLowerCase().startsWith("bring:"));
    const bringItems = bringStep ? bringStep.replace(/^bring:\s*/i, "").split(",").map(s => s.trim()) : [];
    const mainSteps = plan.steps.filter(s => !s.toLowerCase().startsWith("bring:"));
      ...
      <div className="space-y-6 animate-slide-in">
        ...
        <section className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <button onClick={() => setScreen("diagnosis")} className="text-sm text-gray-500 hover:text-gray-900 font-medium">← Back to diagnosis</button>
            <span className="bg-green-800 text-white px-3 py-1 rounded-xl text-xs font-bold">⏱ {plan.estimate}</span>
          </div>
          <div className="p-6">
            <h2 className="text-2xl font-extrabold tracking-tight mb-1">Your action plan</h2>
            <p className="text-sm text-gray-500">Follow these steps exactly and you'll have your CPF.</p>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-100 rounded-2xl p-6">
          <div className="relative">
            {/* Timeline line */}
            <div className="absolute left-[15px] top-4 bottom-4 w-px bg-green-800/15" />
            <div className="space-y-0">
              {mainSteps.map((step, i) => {
                // Detect if step mentions Documents tab or Ready Pack for linking
                const hasDocsRef = step.toLowerCase().includes("documents tab");
                const hasOfficeRef = step.toLowerCase().includes("where to go");
                return (
                  <div key={i} className="flex items-start gap-4 relative py-3">
                    <div className="w-8 h-8 bg-green-800 text-white rounded-full flex items-center justify-center shrink-0 text-xs font-bold z-10 shadow-sm">
                      {i + 1}
                    </div>
                    <div className="flex-1 pt-0.5">
                      <p className="text-sm font-medium leading-relaxed text-gray-900">{step}</p>
                      {hasDocsRef && onGoToDocuments && (
                        <button onClick={onGoToDocuments} className="text-xs font-semibold text-green-800 mt-1.5 hover:underline">
                          Open Documents tab →
                        </button>
                      )}
                      {hasOfficeRef && onGoToDocuments && (
                        <button onClick={onClose} className="text-xs font-semibold text-green-800 mt-1.5 hover:underline">
                          Open Where to go tab →
                        </button>
                      )}
                    </div>
                  </div>
        ...
        {bringItems.length > 0 && (
          <section className="bg-green-800/5 border border-green-800/15 rounded-2xl p-6">
            <h3 className="font-bold text-sm mb-3 flex items-center gap-2">
              <span className="w-6 h-6 bg-green-800 text-white rounded-full flex items-center justify-center text-xs">📋</span>
              What to bring
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {bringItems.map((item, i) => (
                <div key={i} className="flex items-center gap-2 bg-white rounded-xl px-3 py-2.5 border border-green-800/10">
                  <span className="text-green-800 font-bold text-xs">✓</span>
                  <span className="text-sm font-medium text-gray-900">{item.charAt(0).toUpperCase() + item.slice(1)}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        <div className="bg-gray-50 border border-gray-100 rounded-2xl p-4 text-center">
          <p className="text-xs text-gray-500">Most rejections are fixed on the next visit. You've got this.</p>
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 bg-green-800 text-white px-4 py-3.5 rounded-xl font-bold text-sm hover:bg-green-900 transition-all"
          >
            Back to my guide →
          </button>
        </div>
    ...
    return (
      <div className="space-y-6 animate-slide-in">
        <section className="bg-white border border-gray-100 rounded-3xl p-8">
          <button onClick={() => setScreen("triage")} className="text-sm text-gray-500 hover:text-gray-900 mb-4 font-medium">← Change answers</button>
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <section className="bg-white border border-gray-100 rounded-3xl p-8">
        <button onClick={onClose} className="text-sm text-gray-500 hover:text-gray-900 mb-4 font-medium">← Back to guide</button>
          ...
          {LOCATIONS.map((loc) => (
            <button
              ...
              className={`flex items-center gap-3 p-4 rounded-xl text-left transition-all border ${
                location === loc.id
                  ? "bg-green-800/5 border-green-800/30 ring-2 ring-green-800/20"
                  : "bg-gray-50 border-gray-100 hover:border-green-800/20"
              }`}
            >
      ...
      <button
        ...
        className="w-full bg-green-800 text-white px-6 py-3.5 rounded-xl font-bold text-sm hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
      >
        Next →
      </button>
    </div>
